...
<div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
    ...
    <div class="p-4 border-b border-slate-100 bg-slate-50/30 flex flex-col sm:flex-row gap-4 justify-between items-center">
        <form action="{{ route('admin.mahasiswa.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3 w-full justify-between">
            ...
            <div class="flex gap-2">
                <div class="relative w-full sm:w-64">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" class="bg-white border border-slate-200 text-slate-800 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 p-2 shadow-sm" placeholder="Cari NIM, Nama, Prodi...">
                </div>
                ...
                @if(request('search'))
                    <a href="{{ route('admin.mahasiswa.index') }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors shadow-sm" title="Reset Pencarian">
                        <i class="ph ph-x"></i>
                    </a>
                @endif
            </div>
