...
<head>
    ...
    <title>Dosen - SmartClass QR</title>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden relative h-full">
                ...
                <div class="p-6 flex items-center justify-center border-b border-slate-50">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-violet-500 rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-lg shadow-violet-200">
                            <i class="ph ph-qr-code"></i>
                        </div>
                        <h1 class="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-violet-500 to-fuchsia-600">
                            DosenPanel
                        </h1>
                    </div>
                </div>
                ...
                <nav class="flex-1 overflow-y-auto p-4 space-y-1.5 scrollbar-hide">
                    @php
                        $navItems = [
                            ['route' => 'dosen.dashboard', 'icon' => 'ph-squares-four', 'label' => 'Dashboard'],
                            ['route' => 'dosen.jadwal_kuliah', 'icon' => 'ph-calendar-blank', 'label' => 'Jadwal Kuliah'],
                        ];
                    @endphp
                    ...
                    @foreach($navItems as $item)
                        ...
                        <a href="{{ route($item['route']) }}" 
                           class="flex items-center gap-3 px-4 py-3 rounded-2xl transition-all duration-300 group {{ $isActive ? 'bg-violet-500 text-white shadow-md shadow-violet-200 translate-x-1' : 'text-slate-500 hover:bg-violet-50 hover:text-violet-600' }}">
                            <i class="ph {{ $item['icon'] }} text-2xl {{ $isActive ? 'text-white' : 'text-slate-400 group-hover:text-violet-600' }} transition-colors"></i>
        ...
        <main class="flex-1 flex flex-col h-screen overflow-hidden">
            ...
            <header class="h-20 px-4 md:px-8 flex items-center justify-between backdrop-blur-md bg-white/70 border-b border-slate-100/50 z-10 sticky top-0">
                <div class="flex items-center gap-4">
                    <button @click="sidebarOpen = true" class="md:hidden w-10 h-10 flex items-center justify-center rounded-xl bg-white shadow-sm text-slate-500 hover:text-violet-600 transition-colors">
                        <i class="ph ph-list text-2xl"></i>
                    </button>
                    
                    <div class="hidden sm:block">
                        <h2 class="text-xl font-bold text-slate-800">@yield('title', 'Dashboard Dosen')</h2>
                    </div>
                </div>

                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-3 bg-white px-3 py-1.5 md:px-4 md:py-2 rounded-full shadow-sm border border-slate-100">
                        <div class="w-8 h-8 rounded-full bg-violet-100 flex items-center justify-center text-violet-600 font-bold">
